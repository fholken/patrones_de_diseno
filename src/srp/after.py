...
class StripePaymentProcessor:
    def process_transaction(self, customer_data, payment_data) -> dict:
        try:
            charge = {
...
class PaymentService:
    ...
    def process_transaction(self, customer_data, payment_data) -> dict:
        try:
            ...
            print("Invalid customer data:", e)
            raise e

        try:
            self.payment_validator.validate(payment_data)
        except ValueError as e:
            print("Invalid payment data:", e)
            raise e

        try:
            charge = self.payment_processor.process_transaction(customer_data, payment_data)
            self.notifier.notify(customer_data)
